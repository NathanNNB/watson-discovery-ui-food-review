...
export default class Matches extends React.Component {
  ...
  /**
   * getsummary - format title, setting backgroud color for all 
   * highlighted words.
   */
  getsummary(item) {
    if (item.highlight.showHighlight && item.highlight.summaryIndexes.length > 0) {
      var str = '<style>hilite {background:#ffffb3;}</style>';
      item.highlight.summaryIndexes.forEach(function(element) {
          ...
          <List.Item>
            <List.Content>
              <List.Header>
                <List.Description>
                  <h1>
                    <span dangerouslySetInnerHTML={{__html: this.getsummary(item)}}></span>
                  </h1>
                </List.Description>
              </List.Header>
            </List.Content>
